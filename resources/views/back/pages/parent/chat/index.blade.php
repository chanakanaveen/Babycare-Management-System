...
<div class="card-box" style="padding: 0;">
    ...
    <a href="{{ route($routeName, $room->id) }}"
       ...
       onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='{{ $unreadCount > 0 ? '#eff6ff' : '' }}'">
        <div style="width: 48px; height: 48px; border-radius: 50%; background: linear-gradient(135deg, #6366f1, #8b5cf6); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 600; font-size: 18px; flex-shrink: 0; margin-right: 14px;">
        ...
        @if($unreadCount > 0)
        <span class="badge badge-primary badge-pill" style="font-size: 12px; min-width: 24px;">{{ $unreadCount }}</span>
        @endif
    </a>
    @endforeach
</div>
