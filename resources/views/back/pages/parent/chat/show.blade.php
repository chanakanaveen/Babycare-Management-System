...
.chat-header { background: linear-gradient(135deg, #6366f1, #8b5cf6); color: #fff; padding: 16px 20px; display: flex; align-items: center; gap: 12px; }
...
.msg-bubble.sent { align-self: flex-end; background: #6366f1; color: #fff; border-bottom-right-radius: 4px; }
...
.chat-input-bar textarea:focus { border-color: #6366f1; box-shadow: 0 0 0 2px rgba(99,102,241,0.15); }
.chat-input-bar .send-btn { width: 40px; height: 40px; border-radius: 50%; background: #6366f1; border: none; color: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 16px; flex-shrink: 0; transition: background 0.2s; }
.chat-input-bar .send-btn:hover { background: #4f46e5; }
...
<div class="chat-container">
    <div class="chat-header">
        <a href="{{ route('parent.chat.index') }}" style="color: #fff; margin-right: 4px;"><i class="fa fa-arrow-left"></i></a>
        <div class="avatar">{{ strtoupper(substr($otherUser->name ?? 'M', 0, 1)) }}</div>
        <div>
            <h5>{{ $otherUser->name ?? 'Midwife' }}</h5>
...
$(function() {
    ...
    var sendUrl = '{{ route("parent.chat.send", $chatRoom->id) }}';
    var messagesUrl = '{{ route("parent.chat.messages", $chatRoom->id) }}';
